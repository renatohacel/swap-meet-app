import { AsignacionTianguisModel } from "../../models/admin/asignacion_tianguis.model.js";
...
export class AsignacionTianguisController {
    ...
    static async insertTianguisToUser(req, res) {
        ...
        try {
            const result = await AsignacionTianguisModel.insertTianguisToUser(id, tianguis, executeBy);
    ...
    static async deleteTianguisFromUser(req, res) {
        ...
        try {
            const result = await AsignacionTianguisModel.deleteTianguisFromUser(id, tianguis, executeBy);
            res.status(201).send(result);
        } catch (error) {
